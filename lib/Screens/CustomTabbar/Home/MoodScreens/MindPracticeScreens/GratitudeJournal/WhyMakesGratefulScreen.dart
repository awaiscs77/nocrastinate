...
class WhyMakesGratefulScreen extends StatefulWidget {
  final String gratefulThing;

  const WhyMakesGratefulScreen({
    Key? key,
    required this.gratefulThing,
  }) : super(key: key);
...
class _WhyMakesGratefulScreenState extends State<WhyMakesGratefulScreen> {
  ...
  Future<void> _saveGratitudeJournalPractice() async {
    if (_isSaving) return;
    ...
    try {
      ...
      if (practiceManager.hasActiveSession) {
        ...
        final success = await practiceManager.updatePracticeData(data: {
          'gratefulThing': widget.gratefulThing,
          'whyGrateful': _textController.text.trim(),
        });
        ...
        final practiceId = await MindPracticeService.saveGratitudeJournalPractice(
          gratefulThing: widget.gratefulThing,
          whyGrateful: _textController.text.trim(),
        );
